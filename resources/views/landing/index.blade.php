...
<style>
  ...
  /* NATIVE 82% ZOOM SCALE ROOT (APPLIED ONLY TO PAGE CONTENT) */
  .fresh-landing-root {
    zoom: 0.82;
    -webkit-zoom: 0.82;
    width: 100%;
...
<div x-data="{ 
    ...
    <!-- SCALED LANDING PAGE CONTENT (82% ZOOM) -->
    ...
    <!-- END SCALED LANDING CONTENT -->

    <!-- UNZOOMED PERFECTLY CENTERED FIXED MODALS (100% SCALE) -->

    <!-- MODAL 1: FORMULIR PENGUSULAN INTERNAL (AJUKAN PIN) -->
    <div x-show="isProposalModalOpen" x-cloak style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 99999; background: rgba(15,23,42,0.75); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; padding: 1.25rem;" @keydown.escape.window="isProposalModalOpen = false">
        <div @click.away="isProposalModalOpen = false" style="background: #FFFFFF; border: 2px solid var(--color-accent-gold); border-radius: 24px; width: 100%; max-width: 580px; max-height: 86vh; overflow-y: auto; padding: 1.75rem; box-shadow: 0 25px 50px rgba(0,0,0,0.4); color: var(--color-text-dark);">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem; border-bottom: 1.5px solid #F1F5F9; padding-bottom: 0.75rem;">
                <div>
                    ...
                    <h3 style="font-size: 1.2rem; font-weight: 900; color: var(--color-emerald-dark); margin-top: 2px;">Ajukan Usulan Riset / Ide Inovasi Baru</h3>
                </div>
                <button type="button" @click="isProposalModalOpen = false" style="background: #F1F5F9; border: none; font-size: 1.1rem; color: #64748B; width: 34px; height: 34px; border-radius: 50%; cursor: pointer;">✕</button>
            </div>

            <form action="{{ route('landing.submit_proposal') }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 0.95rem;">
                @csrf
                <div style="background: var(--color-emerald-mint); border: 1.5px solid var(--color-emerald-light); padding: 0.65rem 0.85rem; border-radius: 10px; font-size: 0.78125rem; color: var(--color-emerald-dark); font-weight: 800;">
                    🔑 Masukkan PIN Angkatan: <span style="color: var(--color-accent-gold-dark);">GAJAHMADA2026</span>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem;">
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Kode PIN Akses *</label>
                        <input type="password" name="batch_pin" value="GAJAHMADA2026" required style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                    </div>

                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Jenis Usulan *</label>
                        <select name="type" required style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem;">
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="Nama Lengkap & Gelar..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                    </div>

                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Satuan Kerja / Asal *</label>
                        <input type="text" name="institution" required placeholder="Kejari / Kejati / Badiklat..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                    </div>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem;">
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Kategori Bidang *</label>
                        <select name="category" required style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;">
                    ...
                    <div>
                        ...
                        <select name="urgency" required style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;">
                ...
                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Judul Usulan Naskah / Inovasi *</label>
                    <input type="text" name="title" required placeholder="Judul lengkap usulan..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                </div>

                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Deskripsi Singkat / Latar Belakang *</label>
                    <textarea name="description" rows="2" required placeholder="Jelaskan ringkasan masalah dan usulan solusi..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;"></textarea>
                </div>

                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Unggah Berkas Naskah / Draf (PDF/Docx)</label>
                ...
                <div style="display: flex; justify-content: flex-end; gap: 0.65rem; margin-top: 0.4rem;">
                    <button type="button" @click="isProposalModalOpen = false" style="background: #F1F5F9; border: 1px solid #CBD5E1; color: #475569; font-weight: 800; padding: 0.55rem 1.15rem; border-radius: 8px; cursor: pointer; font-size: 0.8125rem;">Batal</button>
                    <button type="submit" style="background: linear-gradient(135deg, var(--color-emerald-dark) 0%, var(--color-emerald-medium) 100%); color: var(--color-accent-gold); font-weight: 900; padding: 0.55rem 1.35rem; border-radius: 8px; border: none; cursor: pointer; font-size: 0.8125rem; box-shadow: 0 4px 14px rgba(27,67,50,0.25);">
                        ⚡ Submit Usulan (PIN)
                    </button>
                </div>
            </form>
    ...
    <div x-show="isCurriculumUploadModalOpen" x-cloak style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 99999; background: rgba(15,23,42,0.75); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; padding: 1.25rem;" @keydown.escape.window="isCurriculumUploadModalOpen = false">
        <div @click.away="isCurriculumUploadModalOpen = false" style="background: #FFFFFF; border: 2px solid var(--color-accent-gold); border-radius: 24px; width: 100%; max-width: 540px; max-height: 86vh; overflow-y: auto; padding: 1.75rem; box-shadow: 0 25px 50px rgba(0,0,0,0.4); color: var(--color-text-dark);">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem; border-bottom: 1.5px solid #F1F5F9; padding-bottom: 0.75rem;">
                <div>
                    <span style="font-size: 0.71875rem; font-weight: 900; color: #16A34A; text-transform: uppercase;">MODUL KURIKULUM PPPJ</span>
                    <h3 style="font-size: 1.2rem; font-weight: 900; color: var(--color-emerald-dark); margin-top: 2px;">Upload Berkas Modul Pembelajaran</h3>
                </div>
                <button type="button" @click="isCurriculumUploadModalOpen = false" style="background: #F1F5F9; border: none; font-size: 1.1rem; color: #64748B; width: 34px; height: 34px; border-radius: 50%; cursor: pointer;">✕</button>
            </div>

            <form action="{{ route('landing.curriculums.upload') }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 0.95rem;">
                @csrf
                <input type="hidden" name="batch_pin" value="GAJAHMADA2026" />

                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Nama Pengunggah *</label>
                    <input type="text" name="uploader_name" required placeholder="Nama Lengkap..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem;">
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Bidang Materi *</label>
                        <select name="subject_category" required style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;">
                    ...
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Tipe Berkas *</label>
                        <select name="file_type" required style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;">
                ...
                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Judul Modul / Materi *</label>
                    <input type="text" name="title" required placeholder="Judul Modul..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                </div>

                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Link External (Google Drive / OneDrive - Optional)</label>
                    <input type="url" name="external_link" placeholder="https://drive.google.com/..." style="width: 100%; background: #F8FAFC; border: 1.5px solid #CBD5E1; border-radius: 8px; padding: 0.55rem 0.75rem; font-size: 0.8125rem; outline: none;" />
                </div>

                <div>
                    <label style="font-size: 0.75rem; font-weight: 800; color: var(--color-text-dark); display: block; margin-bottom: 0.25rem;">Unggah Berkas PDF / PPT</label>
                    <input type="file" name="attachment" style="width: 100%; font-size: 0.78125rem;" />
                </div>

                <div style="display: flex; justify-content: flex-end; gap: 0.65rem; margin-top: 0.4rem;">
                    <button type="button" @click="isCurriculumUploadModalOpen = false" style="background: #F1F5F9; border: 1px solid #CBD5E1; color: #475569; font-weight: 800; padding: 0.55rem 1.15rem; border-radius: 8px; cursor: pointer; font-size: 0.8125rem;">Batal</button>
                    <button type="submit" style="background: linear-gradient(135deg, #16A34A 0%, #15803D 100%); color: #FFFFFF; font-weight: 900; padding: 0.55rem 1.35rem; border-radius: 8px; border: none; cursor: pointer; font-size: 0.8125rem;">
                        📚 Upload Modul
                    </button>
                </div>
            </form>
    ...
    <div x-show="selectedPdf" x-cloak style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 99999; background: rgba(15,23,42,0.8); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; padding: 1.25rem;" @keydown.escape.window="selectedPdf = null">
        <div @click.away="selectedPdf = null" style="background: #FFFFFF; border: 2px solid var(--color-accent-gold); border-radius: 24px; width: 100%; max-width: 860px; height: 82vh; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 25px 50px rgba(0,0,0,0.5);">
            <div style="background: var(--color-emerald-dark); color: #FFFFFF; padding: 1rem 1.5rem; display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <span style="font-size: 0.72rem; color: var(--color-accent-gold); font-weight: 900;" x-text="selectedPdf ? selectedPdf.doc_no : ''"></span>
                    <h3 style="font-size: 1.1rem; font-weight: 900; margin-top: 2px; color: #FFFFFF;" x-text="selectedPdf ? selectedPdf.title : ''"></h3>
                </div>
                <button type="button" @click="selectedPdf = null" style="background: rgba(255,255,255,0.15); border: none; color: #FFFFFF; font-size: 1.1rem; width: 34px; height: 34px; border-radius: 50%; cursor: pointer;">✕</button>
            </div>
            ...
            <div style="background: #FAFDFB; border-top: 1px solid #E2E8F0; padding: 0.85rem 1.5rem; display: flex; justify-content: space-between; align-items: center;">
